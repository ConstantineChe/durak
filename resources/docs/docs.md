...
The `home-routes` handler in the `durak.routes.home` namespace
...
The `home-page` function will in turn call the `durak.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/durak/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `durak.handler` namespace:
...
Request middleware functions are located under the `durak.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `durak.dev-middleware` namespace found in
